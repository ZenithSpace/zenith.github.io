            ...
            <div className="ml-10 flex items-baseline space-x-8 items-center">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="text-gray-300 hover:bg-white/20 hover:text-white hover:backdrop-blur-sm px-3 py-2 rounded-md text-sm font-medium transition-all duration-200"
                >
                  {link.name}
                </a>
              ))}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden bg-zenith-main border-b border-white/10 overflow-hidden"
          >
