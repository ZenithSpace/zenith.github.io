...
const Sponsorship = () => {
    ...
    const HaloEffect = ({ color }: { color: string }) => {
        return (
            <div className="absolute -inset-[100px] -z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none flex items-center justify-center">
                {/* Central Glow */}
                <div
    ...
    const renderTierCard = (tierConfig: any, isDiamond = false) => {
        ...
        return (
            <motion.div
                ...
                whileHover="hover"
                viewport={{ once: true }}
                className={`relative group ${isDiamond ? 'w-full max-w-4xl mx-auto mt-12' : 'h-full'}`}
            >
                {/* Halo Effect Outside */}
